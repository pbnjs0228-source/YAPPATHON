...
let localStream=null,activeCall=null,incoming=null,muted=false,currentUid="";
let stopInvites=null,stopSignals=null,stopPresence=null,stopCallWatch=null,heartbeatTimer=null,pollTimer=null;
const peers=new Map(),queuedIce=new Map(),shownInvites=new Set();
const id=()=>me()?.uid||"";
const callRef=x=>doc(db(),"voiceCalls",x);
const invRef=(x,u)=>doc(db(),"voiceCalls",x,"invites",u);
const presRef=(x,u)=>doc(db(),"voiceCalls",x,"presence",u);
const sigCol=x=>collection(db(),"voiceCalls",x,"signals");
...
function style(){
 ...
 .voice-overlay{position:fixed;inset:0;z-index:10050;background:rgba(0,0,0,.58);backdrop-filter:blur(8px);display:flex;align-items:center;justify-content:center;padding:20px}.voice-overlay[hidden]{display:none!important}
...
function ui(){
 ...
 const r=document.createElement("div");r.id="voice-root";r.innerHTML=`
...
function peer(u){
 ...
 p.onconnectionstatechange=()=>{if(["failed","closed"].includes(p.connectionState)){p.close();peers.delete(u);render()}};
...
function signals(call){
 ...
 stopSignals=onSnapshot(sigCol(call),async snap=>{for(const ch of snap.docChanges()){if(ch.type!=="added")continue;const s=ch.doc.data();if(s.to!==id()||!activeCall)continue;try{
  if(s.type==="offer"){const p=peer(s.from);if(p.signalingState!=="stable")continue;await p.setRemoteDescription(s.offer);for(const c of queuedIce.get(s.from)||[])await p.addIceCandidate(c);queuedIce.delete(s.from);const a=await p.createAnswer();await p.setLocalDescription(a);await addDoc(sigCol(call),{type:"answer",from:id(),to:s.from,answer:{type:a.type,sdp:a.sdp},createdAt:serverTimestamp()})}
  else if(s.type==="answer"){const p=peers.get(s.from);if(p){await p.setRemoteDescription(s.answer);for(const c of queuedIce.get(s.from)||[])await p.addIceCandidate(c);queuedIce.delete(s.from)}}
  else if(s.type==="candidate"){const p=peers.get(s.from);if(p?.remoteDescription)await p.addIceCandidate(s.candidate);else{const q=queuedIce.get(s.from)||[];q.push(s.candidate);queuedIce.set(s.from,q)}}
...
function presenceWatch(call){
 ...
 stopPresence=onSnapshot(collection(db(),"voiceCalls",call,"presence"),async snap=>{for(const ch of snap.docChanges())if(ch.type==="added"&&ch.doc.id!==id()&&activeCall&&!peers.has(ch.doc.id))try{await offer(ch.doc.id)}catch(e){console.warn("Voice offer error",e)}render()})
}
function render(){
 if(!activeCall)return;const name=$("voice-name"),people=$("voice-people");if(!name||!people)return;name.textContent=title(activeCall.context);people.innerHTML="";
 const self=document.createElement("div");self.className="voice-person";self.innerHTML=`<strong>${esc(profile().username||"You")}</strong><span>You</span>`;people.appendChild(self);
 peers.forEach((_,u)=>{const x=document.createElement("div");x.className="voice-person";x.innerHTML=`<strong>${esc(cache()[u]?.username||"Member")}</strong><span>Connected</span>`;people.appendChild(x)});
 const n=peers.size+1;$("voice-count").textContent=n===1?"1 person":`${n} people`
}
function panel(){ui();$("voice-panel").hidden=false;render()}
function watchCall(call){
 if(stopCallWatch)stopCallWatch();
 stopCallWatch=onSnapshot(callRef(call),snap=>{if(!activeCall||activeCall.id!==call)return;if(!snap.exists()){leave();return}const d=snap.data();if(d.active!==true||!fresh(d.updatedAt))leave()})
}
...
async function startCall(){
 ...
 const cid=callId(c),ref=callRef(cid),old=await getDoc(ref);if(old.exists()&&old.data().active&&fresh(old.data().updatedAt)){if(c.type==="channel")return joinCall(cid,c,old.data());toast("A call is already active here.");return}
...
async function joinCall(call,c,data){
 if(activeCall)return toast("You're already in a voice call.");const d=data||((await getDoc(call)).data());if(!d||d.active!==true||!fresh(d.updatedAt)){toast("That voice call has ended.");return}
 try{await mic()}catch(e){toast("Microphone permission is required for voice chat.");return}
...
async function leave(){
 ...
 for(const p of peers.values())p.close();peers.clear();queuedIce.clear();if(stopSignals)stopSignals();stopSignals=null;if(stopPresence)stopPresence();stopPresence=null;if(stopCallWatch)stopCallWatch();stopCallWatch=null;stopHeartbeat();
 ...
 document.querySelectorAll("audio[data-yv]").forEach(a=>{a.srcObject=null;a.remove()});stopMic();muted=false;if($("voice-panel"))$("voice-panel").hidden=true
}
function toggleMute(){muted=!muted;localStream?.getAudioTracks().forEach(t=>t.enabled=!muted);if($("voice-mute"))$("voice-mute").textContent=muted?"🔇 Unmute":"🎙️ Mute"}
function hideIncoming(){incoming=null;if($("voice-incoming"))$("voice-incoming").hidden=true}
function showIncoming(call,data,invite){
 if(activeCall||!invite||invite.status!=="ringing"||!inviteFresh(invite.createdAt)||!fresh(data.updatedAt)||shownInvites.has(call))return;
...
async function accept(){
 if(!incoming)return;const x=incoming;hideIncoming();const d=await getDoc(callRef(x.id));if(!d.exists()||d.data().active!==true||!fresh(d.data().updatedAt)||d.data().createdBy!==x.data.createdBy){toast("That call has ended.");return}await updateDoc(invRef(x.id,id()),{status:"accepted",acceptedAt:serverTimestamp()}).catch(()=>{});await joinCall(x.id,{type:x.data.kind,id:x.data.contextId,name:x.data.name,members:x.data.members||[]},d.data())
}
...
function startInviteListener(){
 ...
 stopInvites=onSnapshot(query(collection(db(),"voiceCalls"),where("members","array-contains",id()),where("active","==",true)),async snap=>{
  for(const ch of snap.docChanges()){
   ...
   const inv=await getDoc(invRef(ch.doc.id,id())).catch(()=>null);if(!inv?.exists())continue;const iv=inv.data();showIncoming(ch.doc.id,d,iv)
  }
 },e=>console.warn("Voice invite listener stopped",e))
}
function button(){
 ui();const h=$("convo-header");if(!h)return;h.querySelectorAll(".voice-start-btn").forEach(x=>x.remove());const c=context();if(!c)return;
