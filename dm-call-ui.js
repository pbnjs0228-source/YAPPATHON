/* YAPPATHON in-chat voice UI v5
   - NEVER removes or disables the incoming-call overlay
   - NEVER uses the old corner panel as call state
   - Hides only the old floating voice panel
   - Shows DM/group calls in the conversation
   - Shows live Ringing state for invitees
*/
(()=>{
'use strict';
...
function hideCorner(){
  document.querySelectorAll('#voice-panel,.voice-panel,#voiceCallPanel,[data-voice-panel]').forEach(e=>{
...
function css(){
 if($('yap-call-v5-style'))return;
 const s=document.createElement('style');s.id='yap-call-v5-style';
 s.textContent=`
 ...
 .voice-dm-tile.ringing{border-color:#f6c85f;animation:yapCallRing 1s ease-in-out infinite}
 ...
 .voice-dm-status{margin-left:auto;color:#8ff0bd;font-size:10px}.voice-dm-status.ringing{color:#ffd36b;font-weight:900}
 ...
 @keyframes yapCallRing{0%,100%{transform:scale(1);box-shadow:0 0 0 0 rgba(246,200,95,.12)}50%{transform:scale(1.025);box-shadow:0 0 0 8px rgba(246,200,95,.25),0 0 28px rgba(246,200,95,.18)}}
...
function tile(u,status,isMe=false){
 ...
 const l=document.createElement('div');l.className='voice-dm-label';const n=document.createElement('span');n.textContent=isMe?(C().profile?.username||'You'):(u?.username||'Member');const st=document.createElement('span');st.className='voice-dm-status'+(status==='ringing'?' ringing':'');st.textContent=status==='ringing'?'🔔 Ringing':status==='connected'?'Connected':'Waiting';l.append(n,st);t.appendChild(l);return t;
}
...
async function subscribe(c){
 ...
 try{
  ...
  stopCall=onSnapshot(doc(db,'voiceCalls',key),snap=>{
   ...
   currentCall=snap.data();render(c);
  },()=>{});
  if(c.type!=='channel'){
...
function render(c){
 ...
 $('voice-dm-sub').textContent=`${online.size||1} connected`;
 ...
 if(c.type==='channel'){
  online.forEach(x=>{if(x!==uid)g.appendChild(tile(user(x),'connected'));});
 }else{
  c.members.filter(x=>x!==uid).forEach(x=>{g.appendChild(tile(user(x),ringing.has(x)?'ringing':online.has(x)?'connected':'waiting'));});
 }
}
...
function update(){
 ...
 if(!c){s.hidden=true;document.querySelector('.main')?.classList.remove('voice-dm-active');if(stopCall)stopCall();if(stopInvites)stopInvites();if(stopPresence)stopPresence();stopCall=stopInvites=stopPresence=null;currentKey='';currentCall=null;return;}
 subscribe(c);
 render(c);
}
function start(){if(timer)return;update();timer=setInterval(update,500);}
